...
import { User, IPhone } from './../../../../models/User';
import {
  AfterViewInit,
  Component,
  ElementRef,
...
export class OrganizationRegistrationComponent
  ...
  passwordregex = /(?=.*[a-z])(?=.*[A-Z])(?=.*[0-9])(?=.*[^A-Za-z0-9])(?=.{8,})/gm;
  phone!: IPhone;
  wizard: any;
  ...
  test() {
    this.registrationForm.valueChanges.pipe(delay(500)).subscribe((result) => {
      switch (this.currentStep) {
        case 1:
          this.registrationForm.controls['accountInfo'].valid
            ? (this.disabled = false)
            : (this.disabled = true);
          break;
        case 2:
          ...
          break;
        case 3:
          this.registrationForm.controls['OrganizationInfo'].valid &&
          this.organizationInfo.controls['agree'].value
            ? (this.disabled = false)
            : (this.disabled = true);
          break;
        default:
      ...
      OrganizationInfo: this.fb.group({
        organizationName: [
          '',
          {
            ...
            asyncValidators: [this.existingInstitutionNameValidator()],
            updateOn: 'blur'
          }
        ],
        location: [
        ...
        website: ['', [UrlValidator(this.utilsService)]],
  ...
  public get Url(): FormControl {
    return this.registrationForm.get('OrganizationInfo.website') as FormControl;
  }
  ...
  public get organizationInfo(): FormGroup {
    return this.registrationForm.controls['OrganizationInfo'] as FormGroup;
  }
  ...
  public get OrganizationLocation(): FormControl {
    return this.registrationForm.get(
      'OrganizationInfo.location'
    ) as FormControl;
  }
  public get OrganizationDomain(): FormControl {
    return this.registrationForm.get('OrganizationInfo.domain') as FormControl;
  }
  public get OrganizationWebsite(): FormControl {
    return this.registrationForm.get('OrganizationInfo.website') as FormControl;
  }
  /**
  ...
  submit() {
    ...
    let organization = {
      name: this.OrganizationName.value.trim().toLowerCase(),
      location: this.OrganizationLocation?.value.trim().toLowerCase(),
      domain: this.OrganizationDomain?.value.trim().toLowerCase(),
      website: this.OrganizationWebsite?.value.trim().toLowerCase(),
      type: this.authService.getTypeOrganization()
    };
    let request = {
      admin: admin,
  ...
  showSpinnerInstitutionName: boolean = false;
  existingInstitutionNameValidator(): AsyncValidatorFn {
    return (control: AbstractControl): Observable<ValidationErrors | null> => {
      if (control && control.value !== '') {
        this.showSpinnerInstitutionName = true;
        return this.authService.ValidateOrganizationName(control.value).pipe(
          map((resp) => {
            this.showSpinnerInstitutionName = false;
            return resp.data
              ? { alreadyTaken: 'Institution name already taken' }
              : null;
          })
        );
      }
